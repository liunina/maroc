...
@implementation MCServiceManager
...
- (void)registerLocalServices {
    NSString *serviceConfigName = [MCContext shareInstance].serviceConfigName;
...
- (id)createService:(Protocol *)service withServiceName:(NSString *)serviceName shouldCache:(BOOL)shouldCache {
    ...
    if (shouldCache) {
        id protocolImpl = [[MCContext shareInstance] getServiceInstanceFromServiceName:serviceStr];
        if (protocolImpl) {
            return protocolImpl;
        }
    }
    ...
    if ([[implClass class] respondsToSelector:@selector(singleton)]) {
        if ([[implClass class] singleton]) {
            ...
            if (shouldCache) {
                [[MCContext shareInstance] addServiceWithImplInstance:implInstance serviceName:serviceStr];
                return implInstance;
            } else {
                return implInstance;
            }
...
- (id)getServiceInstanceFromServiceName:(NSString *)serviceName {
    return [[MCContext shareInstance] getServiceInstanceFromServiceName:serviceName];
}

- (void)removeServiceWithServiceName:(NSString *)serviceName {
    [[MCContext shareInstance] removeServiceWithServiceName:serviceName];
}
